...
//     restaurantDate: restaurant.dinedDate,
...
let generateData = () => {
    ...
    const makeData = (id) => (
      {
        ...
        rating: getRandomInt(1,1),
        review: getBetween3To5Reviews(),
      }
    )
    ...
    const saveDataArray = () => {
      ...
      db.Restaurant.collection.insert(data, (err, resp) => {
        // db.Restaurant.save(data, (err, resp) => {
          console.log (`saved ${num} db entries in this time: `, process.hrtime(t));
        // if (err) {
    ...
    for (var i = 1000001; i <= 1400000; i++) {
      var num = i
      data.push(makeData(i));
    }
    saveDataArray();
  }
